...
static inline ivslam_client_helper_t *IvSlamClient_Create() {
    ...
    if (!me) return NULL;
    __android_log_print(ANDROID_LOG_ERROR, "InvisionSlamClient","IvSlamClient_Create start");
...
//    initContext(&me->context);
    ...
    __android_log_print(ANDROID_LOG_ERROR, "InvisionSlamClient","me->ops->ivslam_getpose error=%s", dlerror());
