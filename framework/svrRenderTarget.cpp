...
namespace Svr {
    ...
    SvrRenderTarget::SvrRenderTarget()
            : mWidth(0), mHeight(0), mSamples(0), mColorAttachmentId(0), mDepthAttachmentId(0),
              mFramebufferId(0) {
        mRefCount = 0;
    }
    ...
    void SvrRenderTarget::InitializeSingleSample(int width, int height, int samples,
                                                 int colorSizedFormat, int format, int type,
                                                 bool requiresDepth) {
        ...
        if (mIsProtectedContent) {
            ...
            GL(glTexImage2D(GL_TEXTURE_2D, 0, colorSizedFormat, width, height, 0, format, type,
                            NULL));
        }
    ...
    void
    SvrRenderTarget::InitializeMultiSample(int width, int height, int samples, int colorSizedFormat,
                                           int format, bool requiresDepth) {
        ...
        GL(glRenderbufferStorageMultisample(renderBuffer, samples, colorSizedFormat, width,
                                            height));
        ...
        if (requiresDepth) {
            ...
            GL(glRenderbufferStorageMultisample(renderBuffer, samples, GL_DEPTH24_STENCIL8, width,
                                                height));
        ...
        GL(glFramebufferRenderbuffer(frameBuffer, GL_COLOR_ATTACHMENT0, renderBuffer,
                                     mColorAttachmentId));
        if (requiresDepth) {
            GL(glFramebufferRenderbuffer(frameBuffer, GL_DEPTH_ATTACHMENT, renderBuffer,
                                         mDepthAttachmentId));
        }
    }

    void SvrRenderTarget::Initialize(int width, int height, int samples, int colorSizedFormat,
                                     bool requiresDepth, bool isProtectedContent) {
        mWidth = width;
        mHeight = height;
        mSamples = samples;
        ...
        if (samples > 1) {
            ...
            InitializeSingleSample(width, height, samples, colorSizedFormat, format, type,
                                   requiresDepth);
        }
    ...
    void SvrRenderTarget::Bind() {
        if (mRefCount != 0) {
            LOGE("Warning! RenderTarget (Handle = %d) is being bound without being unbound. Bind count = %d",
                 mFramebufferId, mRefCount);
        }
    ...
    void SvrRenderTarget::Unbind() {
        if (mRefCount == 0) {
            LOGE("Warning! RenderTarget (Handle = %d) is being unbound without being bound.",
                 mFramebufferId);
        } else {
            mRefCount = mRefCount - 1;
        }
