...
typedef void (*HANDSHANK_KEY_EVENT_CALLBACK)(int keycode, int action, const int lr);
SVRP_EXPORT void scHandShank_SetKeyEventCallback(HANDSHANK_KEY_EVENT_CALLBACK callback);

typedef void (*HANDSHANK_KEY_TOUCH_EVENT_CALLBACK)(bool key1, bool key2, bool key3, bool key4, const int lr);
SVRP_EXPORT void scHandShank_SetKeyTouchEventCallback(HANDSHANK_KEY_TOUCH_EVENT_CALLBACK callback);

typedef void (*HANDSHANK_TOUCH_EVENT_CALLBACK)(int touch_x, int touch_y, const int lr);
SVRP_EXPORT void scHandShank_SetTouchEventCallback(HANDSHANK_TOUCH_EVENT_CALLBACK callback);

typedef void (*HANDSHANK_HALL_EVENT_CALLBACK)(int hall_x, int hall_y, const int lr);
SVRP_EXPORT void scHandShank_SetHallEventCallback(HANDSHANK_HALL_EVENT_CALLBACK callback);

typedef void (*HANDSHANK_CHARGING_EVENT_CALLBACK)(bool isCharging, const int lr);
SVRP_EXPORT void scHandShank_SetChargingEventCallback(HANDSHANK_CHARGING_EVENT_CALLBACK callback);

typedef void (*HANDSHANK_BATTERY_EVENT_CALLBACK)(int battery, const int lr);
SVRP_EXPORT void scHandShank_SetBatteryEventCallback(HANDSHANK_BATTERY_EVENT_CALLBACK callback);

typedef void (*HANDSHANK_CONNECT_EVENT_CALLBACK)(bool isConnected, const int lr);
...
typedef void (*APP_EXIT_CALLBACK)();
SVRP_EXPORT void sc_setAppExitCallback(APP_EXIT_CALLBACK callback);

const char* getInvisionVersion();
typedef void (*GESTURE_CHANGE_CALLBACK)(const int gesture);
typedef void (*GESTURE_MODEL_DATA_CHANGE_CALLBACK)();
typedef void (*LOW_POWER_WARNING_CALLBACK)(const int power);
