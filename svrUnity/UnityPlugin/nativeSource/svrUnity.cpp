...
#define Log(...)		__android_log_print(ANDROID_LOG_INFO, "svrUnity-app", __VA_ARGS__ )
#define LogError(...)	__android_log_print(ANDROID_LOG_ERROR, "svrUnity-app",__VA_ARGS__ )
...
#define GL_TEXTURE_FOVEATED_FEATURE_QUERY_QCOM          0x8BFDsvrGetTransformMatrix(bool &outBLoaded, float *outTransformArray, float *outCorrectQuat)
...
void SvrInitializeEvent()
{
	...
	{
		LogError("SvrInitializeEventData() must be called first before calling SvrInitializeEvent!");
		return;
	}
...
void SvrBeginEyeEvent()
{
    ...
    GLenum err;
    if (glTextureFoveationParametersQCOM != NULL && plugin.textureId > 0 && plugin.IsFoveatedEnabled())
    {
        ...
        if (!bEyeInitialized)
        {
            Log("BeginEye Texture Foveation - textureId %d, previousId %d", plugin.textureId, plugin.previousId);
...
void SvrEndEyeEvent()
{
    ...
    if((glFramebufferFoveationConfigQCOM != NULL) && (glFramebufferFoveationParametersQCOM != NULL) && plugin.IsFoveatedEnabled())
    {
       GLint drawFboId;
       ...
       glFramebufferFoveationParametersQCOM(drawFboId, 0, 0, plugin.focalPointX, plugin.focalPointY,
                                           plugin.foveationGainX, plugin.foveationGainY, plugin.foveationArea);
       err = glGetError();
       if (err != GL_NO_ERROR)
       {
...
void SvrFoveationEvent()
{
    ...
    if (glTextureFoveationParametersQCOM != NULL && plugin.textureId > 0 && plugin.IsFoveatedEnabled())
    {
        glTextureFoveationParametersQCOM(plugin.textureId, 0, 0, plugin.focalPointX, plugin.focalPointY,
            plugin.foveationGainX, plugin.foveationGainY, plugin.foveationArea);
...
SVR_EXPORT void SvrSubmitFrameEventData(int frameIndex, float fieldOfView = 0/*use device fov*/, int frameType = 1/*kEyeBufferStereoSeparate*/)
{
...
SVR_EXPORT void SvrSetFoveationParameters(int textureId, int previousId, float focalPointX, float focalPointY, float foveationGainX, float foveationGainY, float foveationArea, float foveationMinimum)
{
...
SVR_EXPORT void SvrGetDeviceInfo(int &displayWidthPixels, int &displayHeightPixels, float &displayRefreshRateHz,
                                ...
                                float &leftEyeFrustumLeft, float &leftEyeFrustumRight, float &leftEyeFrustumBottom, float &leftEyeFrustumTop, float &leftEyeFrustumNear, float &leftEyeFrustumFar,
                                float &rightEyeFrustumLeft, float &rightEyeFrustumRight, float &rightEyeFrustumBottom, float &rightEyeFrustumTop, float &rightEyeFrustumNear, float &rightEyeFrustumFar,
...
SVR_EXPORT void SvrSetupLayerData(int layerIndex, int sideMask, int textureId, int textureType, int layerFlags)
{
...
SVR_EXPORT void SvrSetupLayerCoords(int layerIndex, float lowerLeft[], float lowerRight[], float upperLeft[], float upperRight[])
{
...
//};
SVR_EXPORT bool SvrPollEvent(int &eventType, unsigned int &deviceId, float &eventTimeStamp, int eventDataCount, unsigned int eventData[])
{
    svrEvent frameEvent;
    ...
    if (isEvent)
    {
        LogDebug("SvrPollEvent() type: %d, deviceId: %d, timeStamp: %f, data: %d %d!", frameEvent.eventType, frameEvent.deviceId, frameEvent.eventTimeStamp, frameEvent.eventData.data[0], frameEvent.eventData.data[1]);
...
SVRP_EXPORT float SvrFetchDeflection() {
    return svrFetchDeflection();
}

SVRP_EXPORT void SvrUpdateRelativeDeflection(int deflection) {
    svrUpdateRelativeDeflection(deflection);
}
...
SVR_EXPORT int ScGetGnss(double &dt, float *gnss) {
    scGetGnss(dt, gnss);
    return 1;
}
...
SVRP_EXPORT int ScFetchHandShank(float *orientationArray, int lr) {
    return scFetchHandShank(orientationArray, lr);
}
...
SVR_EXPORT int SvrGetLatestCameraFrameData(bool &outBUpdate, uint32_t &outCurrFrameIndex,
                                              uint64_t &outFrameExposureNano, uint8_t *outFrameData,
                                              float *outTRDataArray) {
    return svrGetLatestCameraDataForUnity(outBUpdate, outCurrFrameIndex, outFrameExposureNano,
                                     outFrameData, outTRDataArray);
}

SVR_EXPORT int SvrGetLatestCameraFrameDataNoTransform(bool &outBUpdate, uint32_t &outCurrFrameIndex,
                                              uint64_t &outFrameExposureNano, uint8_t *outFrameData,
                                              float *outTRDataArray) {
    return svrGetLatestCameraDataForUnityNoTransform(outBUpdate, outCurrFrameIndex, outFrameExposureNano,
                                     outFrameData, outTRDataArray);
}

SVR_EXPORT int SvrGetTransformMatrix(bool &outBLoaded, float *outTransformArray) {
    return svrGetTransformMatrix(outBLoaded, outTransformArray);
}

SVR_EXPORT int SvrGetLatestEyeMatrices(float *outLeftEyeMatrix, float *outRightEyeMatrix, float *outT, float *outR, int frameIndex, bool isMultiThreadedRender) {
    int pipelineDepth = isMultiThreadedRender ? 2 : 1;
    float predictedTimeMs = svrGetPredictedDisplayTimePipelined(pipelineDepth);
    svrHeadPoseState poseState = svrGetLatestEyeMatrices(outLeftEyeMatrix, outRightEyeMatrix, outT, outR, predictedTimeMs);
    if (frameIndex >= 0) {
        int poseIndex = frameIndex % plugin.kPoseCount;
        plugin.headPoses[poseIndex] = poseState;
    }
    return poseState.poseStatus;
}

SVR_EXPORT int SvrGetLatestCameraBinocularData(bool &outBUpdate, uint32_t &outFrameIndex,
                                                  uint64_t &outFrameExposureNano,
                                                  uint8_t *outLeftFrameData,
                                                  uint8_t *outRightFrameData) {
    return svrGetLatestCameraBinocularData(outBUpdate, outFrameIndex, outFrameExposureNano,
                                              outLeftFrameData, outRightFrameData);
}
